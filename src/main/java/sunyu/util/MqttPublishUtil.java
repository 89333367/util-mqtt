...
/**
 * MQTT <b>消息发布端（Producer）</b>工具类 —— 基于 Eclipse Paho MQTT 5 同步客户端 {@link MqttClient} 实现，
 ...
 *   <li>默认值合理：默认 cleanStart=true（即用即走，broker 不保留本端会话）、automaticReconnect=true
 ...
 *   <li>{@code clientId}：未设置时自动生成 {@code pub-<UUID 截断>}，MQTT 5 协议未强制限制长度，
...
public class MqttPublishUtil implements AutoCloseable {
     ...
     *   <li>调用 {@link MqttClient#connect(MqttConnectionOptions)} 阻塞直到收到 CONNACK 或超时抛异常。</li>
    ...
    private MqttPublishUtil(String broker, String clientId, String username, char[] password,
        ...
        try {
            ...
            // reasonCode 对照表：可查阅 MqttException 源码或 MQTT 5 CONNACK 文档
    ...
    /**
     * 同步发送一条字符串消息 —— 适用于发送 JSON、文本指令等场景。
     *
    ...
    public static class Builder {
        ...
        /**
         * cleanStart 标记。发布端默认 true —— 即用即走，broker 不需要为该 clientId 保留会话。
        ...
        /**
         * 设置 cleanStart 标记。发布端默认 true —— 即用即走。
         *
         * <p>改为 false 意味着：
         * <ul>
